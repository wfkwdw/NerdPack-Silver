...
local agonyCycle = {
	{ '!Agony', 'combat & enemy & debuff.duration <= 5.4 + gcd & range <= 40', {'boss1', 'boss2', 'boss3', 'boss4', 'boss5, focus'}},
...
local maleficGrasp = {
	...
	{ 'Reap Souls', '!player.buff(Deadwind Harvester & combat.time > 5 & player.buff(Tormented Souls >= 5'},
	...
	{ '!Agony', '!player.casting(Unstable Affliction) & debuff.duration <= 5.4+gcd & !{ player.lastcast(Unstable Affliction) & debuff(Unstable Affliction).many < 3 }'},
	{ agonyCycle, '!player.casting(Unstable Affliction) & !player.lastcast(Unstable Affliction)'},
	...
	{ '!Siphon Life', '!player.casting(Unstable Affliction) & debuff.duration <= 4.5+gcd & !{ player.lastcast(Unstable Affliction) & debuff(Unstable Affliction).many < 3 }'},
	--actions.mg+=/siphon_life,cycle_targets=1,if=active_enemies>1&remains<=tick_time+gcd&buff.active_uas.stack=0
	{ siphonLifeCycle, 'target.debuff(Unstable Affliction).many = 0'},
	...
	{ '!Corruption', '!player.casting(Unstable Affliction) & { talent(2,2) & !debuff || !talent(2,2) & debuff.duration <= 4.2+gcd} & { target.area(10).enemies < 3 & talent(4,2) || target.area(10).enemies < 5} & !{ player.lastcast(Unstable Affliction) & debuff(Unstable Affliction).many < 3 }'},
	...
	{ corruptionCycle, '!player.casting(Unstable Affliction) & { target.area(10).enemies < 3 & talent(4,2) || target.area(10).enemies < 5 } & { target.debuff(Unstable Affliction).many = 0 || equipped(132457)'},
	...
	{ agonyCycle, '!player.casting(Unstable Affliction) & target.debuff(Unstable Affliction).many = 0 & !{ player.lastcast(Unstable Affliction) & target.debuff(Unstable Affliction).many = 0 }'},
	...
	{ 'Seed of Corruption', 'talent(4,2) & target.area(10).enemies >= 3 || target.area(10).enemies >= 5'},
	...
	{ '!Unstable Affliction', '!player.casting & { !talent(4,2) || area(10).enemies < 3 } & area(10).enemies < 5 & { player.lastcast & player.soulshards >= 4 & { equipped(132457) || debuff.many < 2 }}', "target"},
	...
	{ '!Unstable Affliction', '!player.casting(Unstable Affliction) & { !talent(4,2) || target.area(10).enemies < 3 } & target.area(10).enemies < 5 & { player.soulshards >= 4 || { equipped(132457 & player.soulshards = 5}}'},
	...
	{ '!Unstable Affliction', '!player.casting(Unstable Affliction) & !equipped(132457) & { player.lastcast & debuff.many < 3 } & debuff.many < 3 & debuff(Agony).duration > player.spell.casttime+6.5 & { debuff(Corruption).duration+6.5 || talent(2,2)} & { !talent(7,2) || debuff(Siphon Life).duration > player.spell.casttime+6.5}'}, 
	...
	{ '!Unstable Affliction', '!player.casting(Unstable Affliction) & { !talent(4,2) ||target.area(10).enemies < 3 } & target.area(10).enemies < 5 & equipped(132457) & { debuff.many = 0 || { player.lastcast & debuff.many != 3} & debuff.many = 1 } & debuff(Agony).duration > player.spell.casttime+6.5'},
	...
	{ '!Drain Soul', 'target.debuff.duration <= gcd'},
	--{ 'Drain Soul'},
	--actions.mg+=/life_tap
	{ 'Life Tap'},
}
